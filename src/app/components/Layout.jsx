...
import { LayoutDashboard, Building2, PlusCircle, Users, FileText, BarChart3, Megaphone, UserCog, Settings as SettingsIcon, Bell, ChevronDown, TrendingUp, LogOut, User, Shield, X, CheckCircle2, AlertCircle, Info, Moon, Sun } from "lucide-react";
...
export function Layout() {
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "dark bg-[#0a0e1a]" : "bg-background"}`}>
      {/* Sidebar */}
      <aside className={`w-64 flex flex-col ${darkMode ? "bg-[#0f1421]" : "bg-sidebar"}`}
        ...
        <div className="p-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white font-bold text-base leading-tight">sarthiDB</div>
              <div className="text-[#93b4ff] text-[10px] leading-tight">sarthiDB (Smart Data Platform)</div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className={`h-16 border-b flex items-center justify-end px-6 shadow-sm ${darkMode ? "bg-[#0f1421] border-gray-800" : "bg-white border-border"}`}>
          <div className="flex items-center gap-2">
            ...
            <button onClick={toggleDarkMode}
              ...
              {darkMode ? <Sun className="w-5 h-5 text-yellow-400" /> : <Moon className="w-5 h-5 text-muted-foreground" />}
            </button>
            ...
            <div ref={notifRef} className="relative">
              ...
              {notifOpen && (
                <div className={`absolute right-0 top-12 w-96 rounded-2xl shadow-2xl border z-50 overflow-hidden ${darkMode ? "bg-[#0f1421] border-gray-700" : "bg-white border-border"}`}>
                  <div className={`flex items-center justify-between px-5 py-4 border-b ${darkMode ? "border-gray-700" : "border-border"}`}>
                    <div>
                      ...
                      {unread > 0 && <p className={`text-xs ${darkMode ? "text-blue-400" : "text-[#4b5e9e]"}`}>{unread} unread</p>}
                    </div>
                    {unread > 0 && (
                      <button onClick={markAllRead} className="text-xs text-[#1e40af] hover:underline font-medium">Mark all read</button>
                    )}
                  </div>
                  <div className={`max-h-80 overflow-y-auto divide-y ${darkMode ? "divide-gray-700" : "divide-border"}`}>
                    {notifList.length === 0 ? (
                      <div className={`p-8 text-center text-sm ${darkMode ? "text-gray-400" : "text-muted-foreground"}`}>No notifications</div>
                    ) : notifList.map(n => (
                      <div key={n.id} className={`flex gap-3 px-5 py-4 group transition-colors ${!n.read ? darkMode ? "bg-blue-900/20" : "bg-blue-50/50" : darkMode ? "hover:bg-[#1a1f2e]" : "hover:bg-muted/40"}`}>
                        <div className="mt-0.5 shrink-0">{notifIcons[n.type]}</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            <p className={`text-sm font-medium truncate ${darkMode ? "text-white" : "text-[#0f172a]"}`}>{n.title}</p>
                          ...
                          <p className={`text-xs mt-0.5 leading-relaxed ${darkMode ? "text-gray-400" : "text-muted-foreground"}`}>{n.body}</p>
            ...
            <div ref={profileRef} className="relative">
              <button onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false); }}
                ...
                <div className="hidden sm:block text-left">
                  <div className={`text-sm font-medium leading-tight ${darkMode ? "text-white" : "text-[#0f172a]"}`}>{user?.name ?? "Guest"}</div>
                  <div className={`text-[10px] leading-tight ${darkMode ? "text-gray-400" : "text-muted-foreground"}`}>{user?.role ?? ""}</div>
                </div>
                <ChevronDown className={`w-4 h-4 transition-transform ${profileOpen ? "rotate-180" : ""} ${darkMode ? "text-gray-400" : "text-muted-foreground"}`} />
              </button>

              {profileOpen && (
                <div className={`absolute right-0 top-12 w-72 rounded-2xl shadow-2xl border z-50 overflow-hidden ${darkMode ? "bg-[#0f1421] border-gray-700" : "bg-white border-border"}`}>
                  <div className="px-5 py-4 bg-gradient-to-br from-[#0f2463] to-[#1e40af]">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="inline-block mt-1 px-2 py-0.5 rounded-full bg-blue-500/40 text-blue-100 text-[10px] font-medium">{user?.role}</span>
                      </div>
                    </div>
                  </div>
                  <div className={`px-5 py-4 space-y-2 border-b ${darkMode ? "border-gray-700" : "border-border"}`}>
                    <p className={`text-xs font-semibold uppercase tracking-wide ${darkMode ? "text-gray-400" : "text-muted-foreground"}`}>Profile Information</p>
                  ...
                  <div className="p-2">
                    <button onClick={() => { navigate("/settings"); setProfileOpen(false); }}
                      ...
                      <SettingsIcon className={`w-4 h-4 ${darkMode ? "text-gray-400" : "text-muted-foreground"}`} /> App Settings
                    </button>
                    <button onClick={() => { navigate("/security"); setProfileOpen(false); }}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors text-sm ${darkMode ? "hover:bg-[#1a1f2e] text-white" : "hover:bg-muted text-[#0f172a]"}`}>
                      <Shield className={`w-4 h-4 ${darkMode ? "text-gray-400" : "text-muted-foreground"}`} /> Security
                    </button>
                    <button onClick={() => { navigate("/account"); setProfileOpen(false); }}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors text-sm ${darkMode ? "hover:bg-[#1a1f2e] text-white" : "hover:bg-muted text-[#0f172a]"}`}>
                      <User className={`w-4 h-4 ${darkMode ? "text-gray-400" : "text-muted-foreground"}`} /> Account
                    </button>
      ...
      {/* AI Assistant */}
      <SarthiDBAssistant darkMode={darkMode} />
    </div>
  );
}
